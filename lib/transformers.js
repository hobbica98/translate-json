...
const google = ({ from = required("from"), to = required("to"), preserveEntities }) => async (
  value
) => {
  const valueWithDecodedEntities = decode(value)
  console.log(valueWithDecodedEntities, from, to)
  try {
...
const yandex = ({
  ...
  try {
    const [text] = await translator.translate(processedValue, `${from}-${to}`)
